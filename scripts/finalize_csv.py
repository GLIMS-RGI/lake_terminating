...
def _argparser():
    ...
    parser.add_argument('fn_outlines', action='store', type=str,
                        help='The filename of the outlines shapefile to convert.')
...
def main():
    ...
    fn_out = os.path.basename(args.fn_outlines).split('.shp')[0] + f'_{(args.contributor.lower())}.csv' # the name of the csv file to write out
    ...
    outlines.rename(columns={'term_type': 'auto_term', 'inv_doi': 'inventory_doi', 'cont': 'contributor', 'img_id': 'image_id', 'img_date': 'image_date'}, inplace=True)
